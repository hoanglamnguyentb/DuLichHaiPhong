...
jQuery.fn.extend({
    rating: function (options = {}) {
        return this.each(function () {
            ...
            this.stars = options.value ? options.value : defaultsStar.value;
            this.readonly = options.readonly ? options.readonly : defaultsStar.readonly;
            ...
            $(this).css({
                "color": options.color ? options.color : defaultsStar.color
            })
                .attr("rating", true);

            if (!this.readonly) {
                $(this).off('mousemove').on('mousemove', function (e) {
                    let halfStars = options.half ? options.half : defaultsStar.half;

                    if (this.getStars().index(e.target) >= 0) {
                        if (!halfStars) {
                            $(this).find("i").attr("class", options.emptyStar ? options.emptyStar : defaultsStar.emptyStar);
                            let index = this.getStars().index(e.target) + 1;

                            for (let i = 0; i < this.getStars().length; i++) {
                                if (i < index)
                                    $(this.getStars()[i]).attr("class", options.filledStar ? options.filledStar : defaultsStar.filledStar)
                            }
                            ...
                            for (let i = 0; i < this.getStars().length; i++) {
                                if (i + 0.5 < index) {
                                    ...
                                    $(this.getStars()[i]).attr("class", options.halfStar ? options.halfStar : defaultsStar.halfStar)
                                }
                            }
                        }
                    }
                ...
                $(this).off('click').on('click', function (e) {
                    ...
                    const callback = options.click ? options.click : defaultsStar.click;
                    callback({
                        "stars": this.stars,
                        "event": e
                    });
            ...
            const stars = options.stars ? options.stars : defaultsStar.stars;
            for (let i = 0; i < stars; i++) {
                let star = $("<i></i>")
                    .addClass(options.emptyStar ? options.emptyStar : defaultsStar.emptyStar)
                    .appendTo($(this));
